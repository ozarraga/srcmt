...
class sfGuardFormSignin extends BasesfGuardFormSignin {
	...
	protected function asignarEtiquetas() {
		...
		if (sfConfig::get('app_sf_guard_plugin_allow_login_with_email', true)) {
			$this->widgetSchema['username']->setLabel('Cuenta de Usuario o Correo Electr&oacute;nico');
		}else {
			$this->widgetSchema['username']->setLabel('Cuenta de Usuario');
		}
...
//		);
	...
	protected function asignarMensajesValidadores() {
		...
		if ($this->getValidatorSchema()->getPostValidator() instanceof sfGuardValidatorUser) {

			$this->getValidatorSchema()->getPostValidator()->setMessages(
				array_merge($this->getValidatorSchema()->getPostValidator()->getMessages(), array(
				    'invalid' => 'Usuario/Password inv&aacute;lidos.'
				)));
		}
		if ($this->getValidatorSchema()->getPostValidator() instanceof sfValidatorCSRFToken) {

			$this->getValidatorSchema()->getPostValidator()->setMessages(
				array_merge($this->getValidatorSchema()->getPostValidator()->getMessages(), array(
				    'csrf_attack' => 'Ataque CSRF Detectado'
				)));
		}
	}
