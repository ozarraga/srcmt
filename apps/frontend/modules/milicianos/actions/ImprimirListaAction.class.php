...
class ImprimirListaAction extends sfAction {
    ...
    public function execute($request) {
        ...
        $tituloReporte = "Listado de Brigadistas Estudiantiles.";

        $consulta = $query->getSqlQuery();

        $srcmt_milicianos = $query->execute();
        $output = array();
        foreach ($srcmt_milicianos as $i => $aRow) {
            $row = array();
        ...
        unset($srcmt_milicianos, $output);

        // variables para subtitulos de fechas "desde" y "hasta"
        $createdAt = $srcmt_milicianos_filter['created_at'];
        $updatedAt = $srcmt_milicianos_filter['updated_at'];
        $CreadoDesdeFecha = '';
        $CreadoHastaFecha = '';
        ...
        $doc_title = "Listado de Brigadistas Estudiantiles";
        $doc_subject = "Listado de Brigadistas Estudiantiles";
        $doc_keywords = "Brigadistas Estudiantiles, Milicianos";
...
//		$pdf = new sfTCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true);
        ...
        if ($AnchoMinimoDeColumna > $AnchoMaximoDeColumna) {
            ...
            $this->redirect('milicianos/index');
        };
        ...
        $pdf->Output('Brigadistas_estudiantiles.pdf', 'I');
